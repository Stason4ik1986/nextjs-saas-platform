...
export const getAllCompanions = async ({ limit = 10, page = 1, subject, topic }: GetAllCompanions) => {
  ...
  const { data: bookmarks } = await supabase.from('bookmarks').select().eq('user_id', userId).in('companion_id', companionIds); // Notice the in() function used to filter the bookmarks by array
...
export const getCompanion = async (id: string) => {
  ...
  if (error) throw new Error(`Failed to fetch companion: ${error.message}`);
...
export const addToSessionHistory = async (companionId: string) => {
  ...
  if (error) throw new Error(`Failed to store session history: ${error.message}`);
...
export const newCompanionPermissions = async () => {
  ...
  if (error) throw new Error(`Failed to fetch companion permissions ${error.message}`);
...
export const getBookmarkedCompanions = async (userId: string) => {
  ...
  if (error) throw new Error(`Failed to fetch recent bokmark companions ${error.message}`);
